...
class AutoTestCls():
    def __init__(self, opt):
        ...
        # def dataform2
        data_df_diff = np.arange(1, 10).reshape((1, 9))
        self.data_df_diff = pd.DataFrame(data_df_diff)
        self.data_df_diff .columns = ['index', 'spFile', 'logFile', 'outFile', 'LinuxRefoutFile', 'SimulatorStat',
            ...
            for filepath, dirnames, filenames in os.walk(self.test_dir, topdown=False):
                for filename in filenames:
                    if self.is_netlist(filename):
                        ...
                        self.data_df_diff.loc[self.spfile_Num] = [spfile, logFile, outFile, ref_file, SimulatorStat,
    ...
    def run_simulation(self, spfileID):
        ...
        if spfile and self.UPDATE_TAG:
            ...
            RunCmd = self.sh + "simulator {} -f nutascii > {}".format(spfile, logfile)
    ...
    def CompareBinaryFile(self, outfile, binaryoutfile):
        results_1_dict = self.output_file_parser(outfile)
    ...
    def diffout(self):
        for j in range(1, self.spfile_Num+1):
            if self.data_df_diff.loc[j].SimulatorStat and self.data_df_diff.loc[j].LinuxRefoutFile:
                ...
                if binary_out:
                    compare, com_result = self.CompareBinaryFile(outfile, binaryoutfile)
                else:
                    compare = None
                    com_result = str({})
                    if os.path.exists(outfile) and os.path.exists(LinuxRefoutfile):
                        results_1_dict = self.output_file_parser(outfile)
                        results_2_dict = self.output_file_parser(LinuxRefoutfile)
...
if __name__ == '__main__':
    ...
    writer1 = pd.ExcelWriter('data_df_simulator_all.xlsx')
    atc.data_df_simulator.to_excel(writer1)
    writer1.save()
    print("start diff out file...")
    atc.diffout()
    writer2 = pd.ExcelWriter('data_df_diff_all.xlsx')
