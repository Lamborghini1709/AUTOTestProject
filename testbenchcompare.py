...
class AutoTestCls():
    def __init__(self, opt):
        ...
        self.check_nodes = {
            ...
            2: ["Oscillator Period and Frequency--period", "Oscillator Period and Frequency--frequency",
                "Harmonic Balance Oscillator Steady State Analysis--vout", "Harmonic Balance Oscillator Steady State Spectrum--vout"],
            ...
            4: ["Harmonic Balance Steady State Analysis--RF", "Harmonic Balance Steady State Spectrum--RF", "Harmonic Balance AC Analysis--RF"],
            ...
            8: ["Transient Analysis--net44", "Periodic Steady State Analysis--net44", "Periodic Steady State Spectrum--net44",
            ...
            18: ["Periodic Steady State Analysis--RFout", "Periodic Steady State Spectrum--RFout", "Periodic Noise Analysis--out",
            ...
            for filepath, dirnames, filenames in os.walk(self.test_dir, topdown=False):
                for filename in filenames:
                    if self.is_netlist(filename):
                        ...
                        self.data_df_simulator.loc[self.spfile_Num] = [netfile, logFile, outFile, SimulatorStat, Simulatorcost]

                        self.data_df_diff.loc[self.spfile_Num] = [netfile, logFile, outFile, ref_file, AnalysisType,
                                                                  SimulatorStat, Simulatorcost, Simulatordiff, outdiffdetail]
    ...
    def calc_error(self, index, original_results_dict, new_results_dict):
        ...
        check_nodes = self.check_nodes[caseindex]
    ...
    def diffout(self):
        for j in range(1, self.spfile_Num+1):
            if self.data_df_diff.loc[j].SimulatorStat & os.path.exists(self.data_df_diff.loc[j].outFile):
                ...
                compare = None
                com_result = str({})
                if os.path.exists(outfile) and os.path.exists(outfile):
